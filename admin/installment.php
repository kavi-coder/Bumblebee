...
<div class="content">
    ...
    $start_from = ($page-1) * 5;    
    $result = $conn->prepare("SELECT * FROM installment ORDER BY i_id ASC LIMIT $start_from, 5");
    $result->execute();
    for($i=0; $row = $result->fetch(); $i++){
              ...
              <td><?php echo "LKR ". $row['i_value']?></td>
              <td><?php echo "LKR ". $row['i_balance']?></td>
              <td><?php echo "LKR ". $row['i_mp']?></td>
              <td><?php echo $row['i_year' ] ." Yr/s"?></td>
              <td><?php echo "LKR ". $row['i_dpp']?></td>
